...
async function processResponse(request: NextRequest, { params }: { params: { callSid: string } }) {
  ...
  try {
    // Check if AI processing already started (promise exists)
    const aiPromise = global.pendingAIPromises.get(callSid)

    if (aiPromise) {
      console.log('⚡ AI processing already in progress - awaiting result...')
      const {
      ...
      // Clean up
      global.pendingAIPromises.delete(callSid)
      global.pendingSpeechMap.delete(callSid)

      console.log('✅ AI response retrieved from background processing!')
...
<Response>
  ...
  />
    ...
    // Fallback: No promise found, do synchronous processing
    console.log('⚠️ No AI promise found - processing synchronously...')
    const speechResult = global.pendingSpeechMap.get(callSid)

    console.log('📋 Retrieved speech from memory:', speechResult?.substring(0, 50))

    if (!callSid || !speechResult) {
      console.error('❌ Missing data:', {
        callSid: !!callSid,
        speech: !!speechResult,
        mapSize: global.pendingSpeechMap.size,
      })
      throw new Error('Missing callSid or speech data')
    }

    // Clean up after retrieval
    global.pendingSpeechMap.delete(callSid)
    console.log('🗑️ Cleaned up speech from memory')
